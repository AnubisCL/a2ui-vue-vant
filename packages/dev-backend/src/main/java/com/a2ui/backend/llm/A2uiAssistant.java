...
/**
 * LangChain4j AI Service interface for A2UI assistant.
 *
...
public interface A2uiAssistant {
    ...
    /**
     * System prompt for synchronous chat responses.
     * Instructs the LLM to return responses in A2UI JSON format.
     */
    String SYSTEM_PROMPT = """
        你是一个智能助手，可以使用 A2UI 协议返回丰富的用户界面。
        ...
        ### 添加表格组件
        ```json
        {"type":"component","surfaceId":"main","componentId":"table1","component":{"type":"Table","props":{"columns":[{"key":"name","title":"名称"},{"key":"value","title":"值"}],"dataSource":[{"name":"项目1","value":100},{"name":"项目2","value":200}]}}}
        ```
        ...
        2. 收集条件后，返回图表展示数据
        3. 使用自然的对话语气，在 A2UI JSON 之前或之后添加说明文字
        4. 确保返回的 JSON 格式正确
    ...
    /**
     * Simplified system prompt for streaming responses.
     */
    String STREAMING_SYSTEM_PROMPT = """
        你是一个智能助手，可以使用 A2UI 协议返回丰富的用户界面。
        你需要逐步生成 A2UI JSON 消息，每条消息单独一行。

        可用组件类型：Text, Chart, Form, Table, Divider
        每条消息必须是有效的 JSON 格式。
        """;
     ...
     */
    @SystemMessage(STREAMING_SYSTEM_PROMPT)
