...
│   │   └── package.json
│   │
│   ├── vue-components/          # @a2ui/vue-components - Vue 组件
│   │   ├── src/
│   │   │   ├── components/      # 组件实现
│   │   │   │   ├── common/      # 通用组件 (Badge, Tag, Progress...)
│   │   │   │   ├── container/   # 容器组件 (Card, Tabs, Modal)
│   │   │   │   ├── display/     # 展示组件 (Text, Image, Icon...)
│   │   │   │   ├── input/       # 输入组件 (Button, TextField...)
│   │   │   │   └── layout/      # 布局组件 (Row, Column, List)
│   │   │   ├── composables/     # Vue Composables
│   │   │   │   ├── useA2UI.ts      # 主入口 composable
│   │   │   │   ├── useSurface.ts
│   │   │   │   ├── useDataModel.ts
│   │   │   │   └── useComponentRegistry.ts
│   │   │   ├── utils/           # 工具函数
│   │   │   │   ├── vant-props.ts    # Vant Props 映射
│   │   │   │   └── value-reference.ts
│   │   │   ├── styles/          # 样式文件
│   │   │   └── index.ts
│   │   └── package.json
│   │
│   ├── vue-plugin/              # @a2ui/vue-plugin - Vue 插件
│   │   ├── src/
...
| 文件 | 说明 |
|------|------|
| `packages/core/src/types/components.ts` | 所有组件的 Props 类型定义 |
| `packages/core/src/types/messages.ts` | A2UI 消息协议类型定义 |
| `packages/vue-components/src/composables/useA2UI.ts` | 主入口 composable，管理全局状态 |
| `packages/vue-plugin/src/A2uiRenderer.vue` | 主渲染器，负责渲染组件树 |
| `packages/vue-plugin/src/plugin.ts` | Vue 插件，注册组件和提供依赖注入 |
